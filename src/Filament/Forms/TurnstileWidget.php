...
{
    protected string $view = 'turnstile::filament.forms.components.widget';
    ...
    protected bool $script = true;

    protected string $scriptStack = 'scripts';
    ...
    */
    ...
    {
        // Add the default extra attributes to
        parent::configure()
            ->hidden(static function (Turnstile $turnstile): bool {
                return $turnstile->isDisabled();
            })
            ->evaluate(function (FilamentManager $filament, Turnstile $turnstile) { // @phpstan-ignore-line
    ...
    */
    ...
    public function name(string $name): static
    {
        if ($name !== Turnstile::KEY) {
            $this->dataAttributes['response-field-name'] = $name;
        }
    ...
    */
    ...
    /**
     * Enable or disable injecting the Cloudflare Turnstile script in the header.
     *
     * @return $this
     */
    public function withScript(bool $condition = true, string $stack = 'scripts', array $attributes = []): static
    {
    ...
    /**
     * Disable injecting the Cloudflare Turnstile script in the header.
     *
     * @return $this
     */
    public function withoutScript(): static
    {
        return $this->withScript(false);
    }
    ...
    */
    ...
    public function getExpandedAttributesBag(): ComponentAttributeBag
    {
        $id = $this->getId();

        return $this->getExtraAttributeBag()->merge(
            Collection::make([
                'callback' => "_turnstileToken_$id",
                'expired-callback' => "_turnstileExpired_$id",
                'error-callback' => "_turnstileError_$id",
                ...$this->getDataAttributes(),
            ])
                ->mapWithKeys(static fn (string $value, string $key): array => ["data-$key" => $value])
                ->merge([
                    'class' => 'cf-turnstile',
                    'id' => "turnstile-$id",
                ])
    ...
    public function getScript(): bool
    {
        return $this->script;
    }
    ...
    public function getScriptAttributes(): ComponentAttributeBag
    {
        $bag = (new ComponentAttributeBag($this->scriptAttributes));

        if ($this->isExplicit()) {
            $bag->merge(['explicit' => true]);
        }
